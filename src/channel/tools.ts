...
export function buildInstructions(role: AgentRole, agentName: string): string {
  ...
  return [
    ...
    `Use the \`infra\` tool (read-only — GET only) to look up context: \`GET /tasks/<id>?include=comments,messages\` for both the curated comments and the full correspondence on a task you're working on, \`GET /tasks?status=&queue=\` for related tasks, \`GET /agents\` to see who else is connected — on a PEER row, \`connected\` means the stub session is attached, and \`peer.reachable\` is whether a message would actually go anywhere. Prefer that filtered read over \`GET /board\`, which returns every task's full description and truncates. State changes are the sensei's job — if you need something written, ask via \`reply\`.`,
