 ...
 * Sensei gets send+infra for messaging and state; workers/users get reply.
...
export function defaultPermissions(role: AgentRole): string[] {
  ...
  // Non-sensei roles (worker, user) share a reply-only allowlist.
  return ['mcp__jean__reply', 'Read', 'Glob', 'Grep', 'Edit', 'Write', 'Bash(git:*)']
}
