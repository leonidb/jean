...
- **One concept per page.** A page is about one thing — one subscription, one project, one decision, one person, one topic. **The split trigger is MULTIPLE distinct concepts on one page — not length.** A single coherent concept may be long and must be left whole; size alone is never a reason to split. Apply a size *safety-valve* only when a page exceeds **~350 lines AND has a natural fault line** into separate concepts. When you split, update `index.md` + inbound `[[links]]`.
- **Consolidation is a near-fixed-point — leave stable pages alone.** A page that's already coherent (one concept, well-organized) is left exactly as-is: don't split, merge, re-order, or rewrite it. Split/merge are the *exception*, applied only to a genuine one-concept-per-page violation or a page past the safety-valve. Running consolidation twice on an unchanged wiki must produce the same wiki — never oscillate a page's shape run-to-run.
- **Merge is rare and conservative.** Merge only two pages that clearly cover the *same single concept* and are both thin stubs (e.g. accidental duplicates). Never merge distinct concepts to reduce page count; when unsure, leave them separate.
...
Apply the **Page principles** above as you write: one concept per page, wiki-link every concept reference, keep entity-class pages schema-consistent. Splitting a genuinely **multi-concept** page is a **required** operation here — you are the wiki's only writer, so it MUST be done (update inbound `[[links]]` + `index.md`), never deferred, flagged for approval, or left for a human who can't do it. But split on the multi-concept trigger, **not raw length** — a long single-concept page stays whole; the size safety-valve is ~350 lines with a natural fault line. Scan every page you carry into staging for a real multi-concept violation, but **leave already-coherent pages exactly as they are** (fixed-point) — don't reshape a page that's fine.
...
- **Any genuine multi-concept page to split, or accidental duplicate to merge?** Scan the whole staging dir for one-concept-per-page *violations* (not only pages you touched) — but leave coherent pages exactly as they are (fixed-point); a long single-concept page is not a violation.
